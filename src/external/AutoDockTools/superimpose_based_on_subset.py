...
def get_atoms(mol, list_of_indicies, names_to_use=['N','CA','C'], verbose=False):
    ...
    if not len(list_of_indicies):
        raise ValueError('invalid input: list of indicies is empty!')
    atoms = AtomSet()
    num_res = 0
...
if __name__ == "__main__":
    ...
    if len(refCoords)==0:
        print("NO REF COORDS!!!")
        raise ValueError('Unable to superimpose because no reference coordinates were specified!!!')
    mobAtoms = get_atoms(mobMol, moblist, names_to_use, verbose)
    mobCoords = mobAtoms.coords
    if len(mobCoords)==0:
        print("NO MOB COORDS!!!")
        raise ValueError('Unable to superimpose because no mobile coordinates were specified!!!') 
