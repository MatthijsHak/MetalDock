...
You have to provide one as the second argument' % key);
    ...
    def addModel(self, atoms):
        ...
        coords = [l(x) for x in atoms]
        # Use a for loop to call Atom.addConformation for each atom in the first chain
        for atom, coord in zip(self.mol.chains[0].residues.atoms, coords):
            atom.addConformation(coord)
...
class PdbqParser(PdbParser):
    ...
    def parse_PDB_TORSDOF(self, rec):
        ...
        self.mol.TORSDOF = int(rec.split()[0][1])
